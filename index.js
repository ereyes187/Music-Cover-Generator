...
app.get('/fetch-albums', async (req, res) => {
  try {
    ...
    const response = await fetch(`https://api.spotify.com/v1/search?q=${query}&type=album&market=US`, {
      method: 'GET',
      headers: {
...
app.get('/get-album/:id', async (req, res) => {
  try {
    ...
    const response = await fetch(`https://api.spotify.com/v1/albums/${id}?market=US`, {
      method: 'GET',
      headers: {
        'Authorization': 'Bearer ' + req.access_token,
      },
